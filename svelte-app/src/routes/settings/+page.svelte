<script lang="ts">
  ...
  let _aiProvider: AppSettings['aiProvider'] = $state('openai');
  let _aiApiKey = $state('');
  let _aiModel = $state('');
  ...
  let _swipeRightPrimary: 'archive' | 'delete' = $state('archive');
  let _swipeLeftPrimary: 'archive' | 'delete' = $state('delete');
  let _confirmDelete = $state(false);
  let _swipeCommitVelocityPxPerSec = $state(1000);
  let _swipeDisappearMs = $state(5000);
  let backups = $state<{ key: string; createdAt: number }[]>([]);
  ...
  let ruleKeys = $derived([ ...new Set([ ...quickKeys, ...hourKeys, ...dayKeys, ...weekdayKeys, ...timeKeys, ...persistentKeys ]) ]);

  // Tabs
  let currentTab: 'app' | 'mapping' | 'backups' = $state('app');
  const tabItems = [
    { name: 'App', value: 'app' },
