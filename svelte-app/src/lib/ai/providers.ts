...
export async function aiSummarizeEmail(subject: string, bodyText?: string, bodyHtml?: string, attachments?: GmailAttachment[], threadId?: string): Promise<string> {
  ...
  const prompt = hasBody
    ? `Summarize the email as plain-text '-' bullets, most important first. For attachments, add one bullet each. For app post notifications, include bullets for: App name, Poster’s name, Message summary. Use only info explicitly in the email—no inference or added detail. If unclear/missing, omit. Output only the bullet list, no intro, closing, commentary, or code blocks.`
