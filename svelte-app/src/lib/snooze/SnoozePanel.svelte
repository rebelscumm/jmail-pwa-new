<script lang="ts">
  ...
  let activeTab: 'Quick' | 'Hours' | 'Days' | 'Weekdays' | 'Times' | 'Custom' = $state('Quick');
  let preview: string = $state('');
  let selectedRule: string | null = $state(null);
  ...
  let mapped: Set<string> = $derived(new Set(Object.keys($settings.labelMapping || {}).filter((k) => $settings.labelMapping[k]).map((k) => normalizeRuleKey(k))));
